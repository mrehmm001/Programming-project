...
2. In the `setup()` method create a select DOM element using the p5
   [`createSelect`](https://p5js.org/reference/#/p5/createSelect))
   function and populate the options *programmatically* using the
   company names obtained from the columns of `this.data`.
   - Hint: you need to write a `for` loop.
   - Hint: look up the p5.Table `columns` property.

3. In the `draw()` method change the hard-coded company name to
   instead get the value from the select.
